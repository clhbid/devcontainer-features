...
post_start_fails_with_vscode_forwarded_agent() (
    export SSH_AUTH_SOCK=/tmp/vscode-ssh-auth-0000.sock
    post_start_fails_with_next_steps
)
...
check "post-start.sh fails with next steps against VS Code's own agent" post_start_fails_with_vscode_forwarded_agent
